...
function formuleRules(rules, flows) {
    for (var i = 0; i < rules.length; i++) {
        ...
        switch (x1.rule.type) {
            case 'Temperature':
                x2.rule.func = flows[x2.index].func = 'temp : Temperature(currentTemp' + setConditionRules(x2.rule);
                switch (x3.rule.type) {
                    case 'Relay':
                        rule3 = x3.rule.on_or_off ? 'temp.setRelayON();' : ' temp.setRelayOFF();';
                        break;
                    case 'IR':
                        rule3 = 'temp.getIR().sendCommand(\"' + x3.rule.command + '\");';
                        break;
                }
                break;
            case 'Pir':
                //x1.rule.func = "Movement detector";
                x2.rule.func = 'pir : PIRSensor( getCurrentState() == ' + (x2.rule.value=="1" ? true : false) + ')';
                switch (x3.rule.type) {
                    case 'Relay':
                        rule3 = x3.rule.on_or_off ? 'pir.setRelayON();' : ' pir.setRelayOFF();';
                        break;
                    case 'IR':
                        rule3 = 'pir.getIR().sendCommand(\"' + x3.rule.command + '\");';
                        break;
                }
                break;
        }
